import { ServerTimingHelper } from '@shi-institute/utils';
...
export class ReverseProxyCache<Props> {
  ...
  private async internal__match(
    ...
    const isModifiedCheck = request.headers.get('If-Modified-Since') !== null;
    const shouldBypassCache =
      // browsers always include the no-cache directive in prefetch requests, but we do not want to re-build the cache for ever user's prerender or prefetch
      !isPrerenderOrPrefetch &&
      (request.headers.get('Cache-Control')?.includes('no-cache') ||
        (request.headers.get('Pragma') === 'no-cache' &&
          // Browsers send Pragma no-cache for requests that include If-Modified-Since.
          !isModifiedCheck));
