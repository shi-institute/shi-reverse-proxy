import { originalFetch } from '@shi-institute/scoped-globals';
...
export default {
  async fetch(_request, env, ctx): Promise<Response> {
    ...
    try {
      ...
      proxyQueue.enqueue(proxies.customElements);
...
} satisfies ExportedHandler<Env, unknown, unknown, { adminBarHref?: string }>;
