...
export function customElementsManifestPlugin() {
  const VIRTUAL_MODULE_ID = 'virtual:custom-elements-manifest';
  ...
  const plugin = {
    ...
    async load(id) {
      if (id === RESOLVED_VIRTUAL_MODULE_ID) {
        ...
        return `const manifests = new Map(${JSON.stringify(Object.entries(manifest), null, 2)});
...
async function generateManifest() {
  const allSvelteFiles = glob('./**/*.svelte');
