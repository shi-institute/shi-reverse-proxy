import { ScopedFetch } from '@shi-institute/scoped-globals';
import { hash, kebabCaseToPascalCase, pascalCaseToKebabCase } from '@shi-institute/utils';
...
import * as allCustomElementComponents from './components';
import * as components from './components';
import { setUrlForSsr } from './utils/navigation';

// @ts-ignore - dts() vite plugin does not like this
export { navigationListItemSchema, type TopLevelNavigationListItem } from './components/navigation-bar-parts/NavigationList.svelte';

const scopedFetch = new ScopedFetch();
scopedFetch.dangerous__replaceFetch();

type Components = typeof components;
type ComponentName = keyof Components;

export type AdminBarProps = ComponentProps<typeof components.AdminBar>;
export type ExternalNavigationBarProps = ComponentProps<typeof components.ExternalNavigationBar>;
export type PostCardProps = ComponentProps<typeof components.PostCard>;
export type PostCardGridProps = ComponentProps<typeof components.PostCardGrid>;
export type PrimaryNavigationBarProps = ComponentProps<typeof components.PrimaryNavigationBar>;
export type ProgressRingProps = ComponentProps<typeof components.ProgressRing>;
export type SecondaryNavigationBarProps = ComponentProps<typeof components.SecondaryNavigationBar>;
export type SelectProps = ComponentProps<typeof components.Select>;

export type AllComponentProps = {
  AdminBar: AdminBarProps;
  ExternalNavigationBar: ExternalNavigationBarProps;
  PostCard: PostCardProps;
  PostCardGrid: PostCardGridProps;
  PrimaryNavigationBar: PrimaryNavigationBarProps;
  ProgressRing: ProgressRingProps;
  SecondaryNavigationBar: SecondaryNavigationBarProps;
  Select: SelectProps;
};
...
export async function render<N extends ComponentName>(
  componentName: N,
  options: {
    props?: ComponentProps<Components[N]>;
    slotHTML?: string;
  ...
  if (globals.fetch) {
    renderOutput = await scopedFetch.withFetch(globals.fetch!, async () => {
  ...
  const data =
    ...
    unevalProps((options?.props || {}) as Record<string, unknown>).replace(/</g, '\\u003c') +
    '</script>';
